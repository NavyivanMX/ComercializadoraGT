...
Public Class frmClientesFacturasElectronicas
    ...
    Private Function VALIDADATOS() As Boolean
        ...
        If CBRF.SelectedIndex = 0 Then
            OPMsgError("Favor de seleccionar un régimen fiscal")
            CBRF.Focus()
            Return False
        End If
        Dim WSVER As New TimbrarXML()
        Dim RESP As WSNavy.RespuestaActivarCliente
        RESP = WSVER.ValidarRFC(My.Settings.USUWS, My.Settings.PWDWS, TXTRFC.Text)
    ...
    Private Sub CARGARINICIO()
        ...
        Else
            OPLlenaComboBox(CBRF, LRF, "Select CLAVE, CLAVE +' - '+NOMBRE FROM CSATREGIMENFISCAL WHERE ACTIVO=1 AND MORAL=1", CADENA, "Favor de seleccionar", "")
        End If
        Dim SQL As New SqlClient.SqlCommand("SELECT NOMBRE,NOMBRECOMERCIAL,ACTIVO,USO,REGIMENFISCAL FROM CLIENTES WHERE RFC='" + TXTRFC.Text + "'", CONZ)
    ...
    Private Sub CARGADATOS()
        DGV.DataSource = BDLlenatabla("SELECT * FROM DOMICILIOSCLIENTES WHERE RFC='" + TXTRFC.Text + "'", CADENA)
