Public Class frmCorteCaja
    ...
    Private Sub CARGANOMBRE()
        If Not frmPrincipal.CHECACONX() Then
            Exit Sub
        End If
        Dim SQL As New SqlClient.SqlCommand("SELECT NOMBRE FROM EMPLEADOS WHERE TIENDA='" + frmPrincipal.SucursalBase + "' AND CLAVE=" + CAJERA.ToString, frmPrincipal.CONX)
        ...
        End If
        LECTOR.Close()
        SQL.CommandText = "UPDATE TIENDAS SET NOMCAJERA='" + NOMBRE + "' WHERE CLAVE='" + frmPrincipal.SucursalBase + "'"
        SQL.ExecuteNonQuery()
    End Sub
    ...
    Private Sub BTNCORTE_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNCORTE.Click
        ...
        Dim Q1 As New SqlClient.SqlCommand("SELECT SUM(TOTAL)TOTAL,COUNT(NOTAS)NOTAS FROM TOTALCORTE WHERE ESTADO=1 AND TIENDA='" + frmPrincipal.SucursalBase + "' AND NOCAJA=" + NOCAJA.ToString + " AND FECHA>=@FEC", frmPrincipal.CONX)
        ...
        End If
        '''''''''''''HISTORIAL CORTES'''''''''''''''
        Dim SQLREG As New SqlClient.SqlCommand("SELECT MAX(REGISTRO)REG FROM HISTORIALCORTES WHERE TIENDA='" + frmPrincipal.SucursalBase + "' AND CAJA=" + NOCAJA.ToString + " AND TIPOCORTE='Z'", frmPrincipal.CONX)
        ...
        Dim SQLT As New SqlClient.SqlDataAdapter("SELECT CLAVE,NOMBRE,SUM (EFECTIVO)EFECTIVO FROM VISTATIPOPAGO WHERE TIENDA='" + frmPrincipal.SucursalBase + "' AND NOCAJA=" + NOCAJA.ToString + " AND FECHA>=@FEC GROUP BY CLAVE,NOMBRE", frmPrincipal.CONX)
        ...
        QUER = "SELECT S.NOMBRECOMUN,N.FECHA,N.NOTA NOTAS,N.TOTAL,N.CAJA NOCAJA,T.NOMBRE TIPOPAGO,S.NOMBREFISCAL EMPRESA,S.DIRECCION,S.CIUDAD,S.RFC,S.TELEFONO,S.NOMCAJERA,C.CZ,SUBTOTAL,IVA,ELTOTAL=N.TOTAL,FECHACORTE=GETDATE(),HORACORTE=GETDATE() FROM VCORTES N INNER JOIN TIENDAS S ON N.TIENDA =S.CLAVE INNER JOIN TIPOSPAGOS T ON N.TIPOPAGO =T.CLAVE INNER JOIN CORTES C ON N.TIENDA =C.TIENDA  AND N.CAJA=C.CAJA WHERE N.TIENDA='" + frmPrincipal.SucursalBase + "' AND N.FECHA>=@INI  AND N.FORMA<>'CREDITO' ORDER BY N.NOTA"
        IMPRIMIRREPORTE(REP2, BDLlenatabla(QUER, frmPrincipal.CadenaConexion, FQ.Value, FQ.Value), 1, "")


        Dim SQLZ As New SqlClient.SqlCommand("UPDATE CORTES SET FECHAZ=GETDATE(),CZ=CZ+1 WHERE TIENDA='" + frmPrincipal.SucursalBase + "' AND CAJA=" + NOCAJA.ToString, frmPrincipal.CONX)
        ...
        'IMPRIMIRREPORTE(REP3, BDLlenatabla(QUER, frmPrincipal.CadenaConexion, FQ.Value, FQ.Value), 1, "")
        ...
        'IMPRIMIRREPORTE(REP4, BDLlenatabla(QUER, frmPrincipal.CadenaConexion, FQ.Value, FQ.Value), 1, "")
