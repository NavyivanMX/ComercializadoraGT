Public Class frmCajeras
    ...
    Private Sub CARGAR()
        If CHECAEMPLEADO() Then
            If Not frmPrincipal.CHECACONX() Then
                Exit Sub
            End If
            Dim SQL As New SqlClient.SqlCommand("SELECT PWD FROM CAJERAS WHERE TIENDA='" + frmPrincipal.SucursalBase + "'AND EMPLEADO=" + TXTEMP.Text, frmPrincipal.CONX)
    ...
    Private Function CHECAEMPLEADO() As Boolean
        If Not frmPrincipal.CHECACONX() Then
            Return False
        End If
        Dim SQL As New SqlClient.SqlCommand("SELECT NOMBRE FROM EMPLEADOS WHERE TIENDA='" + frmPrincipal.SucursalBase + "' AND CLAVE=" + TXTEMP.Text, frmPrincipal.CONX)
        Dim LECTOR As SqlClient.SqlDataReader
        LECTOR = SQL.ExecuteReader
        If LECTOR.Read Then
