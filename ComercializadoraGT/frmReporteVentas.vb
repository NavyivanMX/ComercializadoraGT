Public Class frmReporteVentas
    ...
    Private Sub CARGADATOS()
        ...
        QUERY = "SELECT G.NOMBRE GRUPO,P.NOMBRE PRODUCTO,SUM(D.CANTIDAD)CANTIDAD,SUM(D.TOTAL)[VENTA $$] FROM NOTASDEVENTA N INNER JOIN DETALLENOTASDEVENTA D    ON N.TIENDA=D.TIENDA AND N.NOTA=D.NOTA INNER JOIN PRODUCTOS P   ON P.CLAVE=D.PRODUCTO INNER JOIN GRUPOS G   ON G.CLAVE=P.GRUPO WHERE N.FECHA>=@INI AND N.FECHA<=@FIN  "

        QUERY3 = "SELECT T.NOMBRECOMUN TIENDA,G.NOMBRE GRUPO,P.NOMBRE PRODUCTO,SUM(D.CANTIDAD)CANTIDAD,SUM(D.TOTAL)[VENTA $$],FECHAINI='" + DTDE.Value + "',FECHAFIN='" + DTHASTA.Value + "'   FROM NOTASDEVENTA N INNER JOIN DETALLENOTASDEVENTA D    ON N.TIENDA=D.TIENDA AND N.NOTA=D.NOTA INNER JOIN PRODUCTOS P   ON P.CLAVE=D.PRODUCTO INNER JOIN GRUPOS G   ON G.CLAVE=P.GRUPO INNER JOIN TIENDAS T ON T.CLAVE=N.TIENDA WHERE N.FECHA>=@INI AND N.FECHA<=@FIN  "
    ...
    Private Sub CHECATABLA()
        ...
        Dim TOT As Double
        TOT = 0
        Dim QUERY, QUERY2, QUERY3, QUERY4, QUERY5 As String
        QUERY = "SELECT dbo.null2numeric(SUM(D.TOTAL))  FROM NOTASDEVENTA N INNER JOIN DETALLENOTASDEVENTA D ON D.TIENDA=N.TIENDA AND D.NOTA=N.NOTA INNER JOIN PRODUCTOS P ON P.CLAVE=D.PRODUCTO INNER JOIN GRUPOS G ON G.CLAVE=P.GRUPO WHERE N.TIENDA='" + frmPrincipal.SucursalBase + "'  AND N.FECHA>='" + DTDE.Value.Date + "' and N.fecha<='" + DTHASTA.Value.Date.AddDays(1) + "' AND N.TIPOPAGO=2 "
        QUERY2 = "SELECT dbo.null2numeric(SUM(D.TOTAL)) FROM NOTASDEVENTA N INNER JOIN DETALLENOTASDEVENTA D ON D.TIENDA=N.TIENDA AND D.NOTA=N.NOTA INNER JOIN PRODUCTOS P ON P.CLAVE=D.PRODUCTO INNER JOIN GRUPOS G ON G.CLAVE=P.GRUPO WHERE N.TIENDA='" + frmPrincipal.SucursalBase + "'  AND N.FECHA>='" + DTDE.Value.Date + "' and N.fecha<='" + DTHASTA.Value.Date.AddDays(1) + "' AND N.TIPOPAGO=1 "
        QUERY3 = "SELECT dbo.null2numeric(SUM(D.TOTAL)) FROM NOTASDEVENTA N INNER JOIN DETALLENOTASDEVENTA D ON D.TIENDA=N.TIENDA AND D.NOTA=N.NOTA INNER JOIN PRODUCTOS P ON P.CLAVE=D.PRODUCTO INNER JOIN GRUPOS G ON G.CLAVE=P.GRUPO WHERE N.TIENDA='" + frmPrincipal.SucursalBase + "'  AND N.FECHA>='" + DTDE.Value.Date + "' and N.fecha<='" + DTHASTA.Value.Date.AddDays(1) + "' AND N.TIPOPAGO=3 "
        QUERY4 = "SELECT dbo.null2numeric(SUM(D.TOTAL)) FROM NOTASDEVENTA N INNER JOIN DETALLENOTASDEVENTA D ON D.TIENDA=N.TIENDA AND D.NOTA=N.NOTA INNER JOIN PRODUCTOS P ON P.CLAVE=D.PRODUCTO INNER JOIN GRUPOS G ON G.CLAVE=P.GRUPO WHERE N.TIENDA='" + frmPrincipal.SucursalBase + "'  AND N.FECHA>='" + DTDE.Value.Date + "' and N.fecha<='" + DTHASTA.Value.Date.AddDays(1) + "' AND N.TIPOPAGO=4 "
        QUERY5 = "SELECT dbo.null2numeric(SUM(D.TOTAL)) FROM NOTASDEVENTA N INNER JOIN DETALLENOTASDEVENTA D ON D.TIENDA=N.TIENDA AND D.NOTA=N.NOTA INNER JOIN PRODUCTOS P ON P.CLAVE=D.PRODUCTO INNER JOIN GRUPOS G ON G.CLAVE=P.GRUPO WHERE N.TIENDA='" + frmPrincipal.SucursalBase + "'  AND N.FECHA>='" + DTDE.Value.Date + "' and N.fecha<='" + DTHASTA.Value.Date.AddDays(1) + "' AND N.TIPOPAGO=5 "
        If CBGRU.SelectedIndex = 0 Then
            If CBPRO.SelectedIndex = 0 Then
            Else
        ...
        Else
            If CBPRO.SelectedIndex = 0 Then
                QUERY = QUERY + "and G.CLAVE='" + CLAGRU(CBGRU.SelectedIndex) + "' "
                QUERY2 = QUERY2 + "and G.CLAVE='" + CLAGRU(CBGRU.SelectedIndex) + "' "
                QUERY3 = QUERY3 + "and G.CLAVE='" + CLAGRU(CBGRU.SelectedIndex) + "' "
                QUERY4 = QUERY4 + "and G.CLAVE='" + CLAGRU(CBGRU.SelectedIndex) + "' "
                QUERY5 = QUERY5 + "and G.CLAVE='" + CLAGRU(CBGRU.SelectedIndex) + "' "
            Else
                QUERY = QUERY + " AND G.CLAVE='" + CLAGRU(CBGRU.SelectedIndex) + "' AND P.CLAVE='" + CLAPROD(CBPRO.SelectedIndex) + "'  "
                QUERY2 = QUERY2 + " AND G.CLAVE='" + CLAGRU(CBGRU.SelectedIndex) + "' AND P.CLAVE='" + CLAPROD(CBPRO.SelectedIndex) + "'  "
                QUERY3 = QUERY3 + " AND G.CLAVE='" + CLAGRU(CBGRU.SelectedIndex) + "' AND P.CLAVE='" + CLAPROD(CBPRO.SelectedIndex) + "'  "
                QUERY4 = QUERY4 + " AND G.CLAVE='" + CLAGRU(CBGRU.SelectedIndex) + "' AND P.CLAVE='" + CLAPROD(CBPRO.SelectedIndex) + "'  "
                QUERY5 = QUERY5 + " AND G.CLAVE='" + CLAGRU(CBGRU.SelectedIndex) + "' AND P.CLAVE='" + CLAPROD(CBPRO.SelectedIndex) + "'  "
            End If
        End If
        ...
        Dim SQLS As New SqlClient.SqlCommand(QUERY2, frmPrincipal.CONX)
        ...
        Dim SQLS2 As New SqlClient.SqlCommand(QUERY3, frmPrincipal.CONX)
        ...
        Dim SQLS4 As New SqlClient.SqlCommand(QUERY4, frmPrincipal.CONX)
        Dim LEC4 As SqlClient.SqlDataReader
        LEC4 = SQLS4.ExecuteReader
        If LEC4.Read Then
            LBLTRA.Text = LEC4(0)
        End If
        LEC4.Close()

        Dim SQLS5 As New SqlClient.SqlCommand(QUERY5, frmPrincipal.CONX)
        Dim LEC5 As SqlClient.SqlDataReader
        LEC5 = SQLS5.ExecuteReader
        If LEC5.Read Then
            LBLTRA.Text = LEC5(0)
        End If
        LEC5.Close()
    ...
    End Sub
    Private Sub BTNGUARDAR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LBLTCR.Click
        GUARDAR()
    End Sub
    ...
    Private Sub frmReporteVentas_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
