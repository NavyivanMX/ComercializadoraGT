Public Class frmReporteAdeudos
    ...
    Private Sub IMPRIMIR()
        ...
        QUER = "SELECT C.NOMBRE CLIENTE,N.NOTA,N.TOTAL,DBO.ADEUDOCREDITO(N.TIENDA,N.NOTA) ABONOS, (Select Convert(Char(10), N.FECHA,103)) CONSUMO, (Select Convert(Char(10), MAX(A.FECHA),103))[ULTIMO PAGO],DATEDIFF ( dd , n.fecha , getdate()) [DIAS TRANSCURRIDOS], TOTALADEUDO=DBO.TOTALDEBECLIENTE(N.TIENDA,N.CLIENTE),C.DIASCREDITO,DBO.FOLIOFAC('" + frmPrincipal.Serie + "',N.NOTA)Factura FROM NOTASDEVENTACREDITO N LEFT JOIN ABONOSCREDITOS A ON N.TIENDA=A.TIENDA AND N.NOTA=A.NOTA INNER JOIN CLIENTES C ON C.TIENDA=N.TIENDA AND C.CLAVE=N.CLIENTE WHERE N.PAGADO=0 AND N.TIENDA='" + frmPrincipal.SucursalBase + "' AND N.CLIENTE=" + CLACLI(CBCLI.SelectedIndex) + " GROUP BY N.NOTA,N.TOTAL,N.FECHA,C.NOMBRE,N.TIENDA,N.CLIENTE,C.DIASCREDITO "
        MOSTRARREPORTE(REPI, "Reporte de adeudos", BDLlenatabla(QUER, frmPrincipal.CadenaConexion), "Enviar a OneNote 2007")
    End Sub
    Private Sub GUARDAR()
        Dim REPI As New rptAdeudos
