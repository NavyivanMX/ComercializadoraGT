...
Public Class frmFacturar33
    ...
    Private Sub frmFacturar33_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        OPLlenaComboBox(CBNEG, LNEG, "SELECT CLAVE,NOMBRE FROM NEGOCIOS WHERE RFC='" + frmPrincipal.EmisorBase + "' AND CLAVE='" + frmPrincipal.SucursalBase + "'  ORDER BY NOMBRE", CADENA)

        ACTIVAR(True)
        CHECATABLA()

        OPLlenaComboBox(CBFP, LFP, "SELECT CLAVE,NOMBRE FROM CSATFORMAPAGO WHERE ACTIVO=1 ORDER BY NOMBRE", CADENA, "Favor de Seleccionar", "")
        OPLlenaComboBox(CBMP, LMP, "SELECT CLAVE,NOMBRE FROM CSATMETODOPAGO WHERE ACTIVO=1 ORDER BY NOMBRE", CADENA, "Favor de Seleccionar", "")
        OPLlenaComboBox(CBUSO, LUC, "SELECT CLAVE,NOMBRE FROM CSATUSOCOMPROBANTE WHERE ACTIVO=1 ORDER BY NOMBRE", CADENA, "Favor de Seleccionar", "")
        OPLlenaComboBox(CBRF, LRF, "SELECT CLAVE,CLAVE +' - '+NOMBRE FROM CSATREGIMENFISCAL WHERE ACTIVO=1 ORDER BY NOMBRE", CADENA, "Favor de Seleccionar", "")
    ...
    Private Sub CARGADATOS()
        ...
        End If
        ACTIVAR(False)
        Dim USO, REGFIS As String
        USO = ""
        REGFIS = ""
        Dim SQL As New SqlClient.SqlCommand("SELECT NOMBRE,NOMBRECOMERCIAL,VN,USO,REGIMENFISCAL FROM CLIENTES WHERE RFC='" + TXTRFC.Text + "'", CONZ)
        ...
        If TXTRFC.Text.Length = 13 Then
            ...
            OPLlenaComboBox(CBUSO, LUC, "SELECT CLAVE,NOMBRE FROM CSATUSOCOMPROBANTE WHERE ACTIVO=1 AND FISICA=1 ORDER BY NOMBRE", CADENA, "Favor de Seleccionar", "")
        Else
            OPLlenaComboBox(CBUSO, LUC, "SELECT CLAVE,NOMBRE FROM CSATUSOCOMPROBANTE WHERE ACTIVO=1 AND MORAL=1 ORDER BY NOMBRE", CADENA, "Favor de Seleccionar", "")
        ...
        OPLlenaComboBox(CBCALLE, LREG, LDOM, LCP, "SELECT REGISTRO,dbo.DIRCLIENTE(RFC,REGISTRO),CP,CALLE FROM DOMICILIOSCLIENTES WHERE RFC='" + TXTRFC.Text + "' AND ACTIVO=1", CADENA)
        OPCargaX(LRF, CBRF, REGFIS)
        OPCargaX(LUC, CBUSO, USO)
    ...
    Private Sub CBCALLE_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CBCALLE.SelectedIndexChanged
        ...
        TXTCP.Text = LCP(CBCALLE.SelectedIndex)
    End Sub
    ...
    Private Function CARGAPAGONOTA(ByVal RES As String, ByVal NOTA As String, ByVal ESCRE As Boolean) As Boolean
        ...
        If ESCRE Then
            ...
            'OPCargaX(LMP, CBMP, "PPD") TODOS EN PUE
            OPCargaX(LMP, CBMP, "PPD")
            OPCargaX(LFP, CBFP, "99")
        Else
            OPCargaX(LMP, CBMP, "PUE")
            Dim FP As Integer
            Dim TT As Integer
            Dim SQL As New SqlClient.SqlCommand("SELECT TIPOPAGO FROM NOTASDEVENTA WHERE TIENDA='" + RES + "' AND NOTA='" + NOTA + "'", frmPrincipal.CONX)
            Dim LEC As SqlClient.SqlDataReader
            LEC = SQL.ExecuteReader
            If LEC.Read Then
                FP = LEC(0)
            ...
            If FP = 1 Then
                'OPCargaX(LMP, CBMP, 1)
                OPCargaX(LFP, CBFP, "01")
            End If
            If FP = 2 Then
                'OPCargaX(LMP, CBMP, 1)
                OPCargaX(LFP, CBFP, "04")
            End If
            If FP = 6 Then
                'OPCargaX(LMP, CBMP, 1)
                OPCargaX(LFP, CBFP, "28")
            End If

            If FP = 3 Then
                'OPCargaX(LMP, CBMP, 4)
                OPCargaX(LFP, CBFP, "02")
            End If

            If FP = 5 Then
                'OPCargaX(LMP, CBMP, 5)
                OPCargaX(LFP, CBFP, "03")
            End If
            SQL.Dispose()
        End If
        LBLFOL.Text = SERIE() + FOLIO(SERIE()).ToString
    ...
    Private Sub NOTASAGREGADAS()
        ...
        End If
        Dim QUERY As String
        QUERY = "SELECT D.PRODUCTO,SUM(D.CANTIDAD)CANTIDAD,P.NOMBRE DESCRIPCION,PRECIO=DBO.VALORESFAC(D.CANTIDAD,P.FACTORIVA,P.TASAIVA,P.FACTORIEPS,P.TASAIEPS,D.DESCUENTO,D.TOTAL,1),TOTAL=SUM(DBO.VALORESFAC(D.CANTIDAD,P.FACTORIVA,P.TASAIVA,P.FACTORIEPS,P.TASAIEPS,D.DESCUENTO,D.TOTAL,5)),U.NOMBRE UNIDAD,IVA=SUM(DBO.VALORESFAC(D.CANTIDAD,P.FACTORIVA,P.TASAIVA,P.FACTORIEPS,P.TASAIEPS,D.DESCUENTO,D.TOTAL,2)),IEPS=SUM(DBO.VALORESFAC(D.CANTIDAD,P.FACTORIVA,P.TASAIVA,P.FACTORIEPS,P.TASAIEPS,D.DESCUENTO,D.TOTAL,3)),P.TASAIVA,P.TASAIEPS,P.GRUPOIEPS,DESCN=SUM(DBO.VALORESFAC(D.CANTIDAD,P.FACTORIVA,P.TASAIVA,P.FACTORIEPS,P.TASAIEPS,D.DESCUENTO,D.TOTAL,4)),P.FACTORIVA,P.FACTORIEPS,P.CPSSAT,U.CSAT,D.CANTIDAD*P.PESO PESO "
        ...
        QUERY += " INNER JOIN PRODUCTOS P ON D.PRODUCTO=P.CLAVE INNER JOIN UNIDADES U ON P.UVENTA=U.CLAVE WHERE D.TIENDA='" + frmPrincipal.SucursalBase + "' AND ("
        Dim X As Integer
        For X = 0 To LISTANOTAS.Count - 1
        ...
        QUERY += ") GROUP BY D.PRODUCTO,P.NOMBRE,U.NOMBRE,P.TASAIEPS,P.GRUPOIEPS,D.DESCUENTO,D.TOTAL,D.CANTIDAD,P.FACTORIVA,P.FACTORIEPS,P.CPSSAT,U.CSAT,P.TASAIVA,P.PESO "
    ...
    Private Function NOTAACTIVA(ByVal TIENDA As String, ByVal NOTA As Integer, ByVal CRE As Boolean) As Boolean
        ...
        Try
            Dim QUERY As String
            If CRE Then
                QUERY = "SELECT NOTA FROM NOTASDEVENTACREDITO WHERE TIENDA='" + TIENDA + "' AND NOTA=" + NOTA.ToString
            Else
                QUERY = "SELECT NOTA FROM NOTASDEVENTA WHERE TIENDA='" + TIENDA + "' AND NOTA=" + NOTA.ToString
            End If
    ...
    Private Sub GUARDAR()
        ...
        For X = 0 To LISTAESCREDITO.Count - 1
            ...
            End If
        Next
        BDEjecutarSql("UPDATE CLIENTES SET NOMBRE='" + TXTNOM.Text + "',NOMBRECOMERCIAL='" + TXTNCOM.Text + "', USO='" + LUC(CBUSO.SelectedIndex) + "',REGIMENFISCAL='" + LRF(CBRF.SelectedIndex) + "' WHERE RFC='" + TXTRFC.Text + "'", CADENA)
        BDEjecutarSql("UPDATE DOMICILIOSCLIENTES SET CP='" + TXTCP.Text + "' WHERE RFC='" + TXTRFC.Text + "' AND REGISTRO=" + LREG(CBCALLE.SelectedIndex).ToString, CADENA)

        Dim SQL As New SqlClient.SqlCommand("INSERT INTO FACTURAS (RFCEMISOR,SERIE,FOLIO,NEGOCIO,RFCCLIENTE,DOMICILIOCLIENTE,TIPOCOMPROBANTE,METODOPAGO,FORMAPAGO,REFNOTA,SUBTOTAL,IVA,TOTAL,CCLETRA,TIPOFACTURA,NOMBRE,DIGITOSTC,COMENTARIO,CONDICIONESDEPAGO,DESCUENTO,MONEDA,TIPOCAMBIO,NMONEDA,NFORMAPAGO,NMETODOPAGO,NTIPOCOMPROBANTE,NUSOCFDI,VCFD,FECHA) VALUES ('" + frmPrincipal.EmisorBase + "','" + VSERIE + "'," + ELFOLIO.ToString + ",'" + LNEG(CBNEG.SelectedIndex) + "','" + TXTRFC.Text + "','" + LREG(CBCALLE.SelectedIndex).ToString + "','1','0','" + LFP(CBFP.SelectedIndex).ToString + "','" + TXTNOTA.Text + "'," + S.ToString + "," + I.ToString + "," + T.ToString + ",'" + LBLCCL.Text + "',1,'" + TXTNOM.Text + "','" + DIGITOSTC + "','" + TXTCOM.Text + "','" + CONDPAG + "','" + VDESC.ToString + "','1',1,'MXN','" + LFP(CBFP.SelectedIndex) + "','" + LMP(CBMP.SelectedIndex) + "','I','" + LUC(CBUSO.SelectedIndex) + "','" + frmPrincipal.VCFD + "',@FEC)", CONZ)
        SQL.Parameters.Add("@FEC", SqlDbType.DateTime).Value = Now
        SQL.ExecuteNonQuery()
        Dim SQLD As New SqlClient.SqlCommand("INSERT INTO DETALLEFACTURAS (RFCEMISOR,SERIE,FOLIO,REFNOTA,CODIGO,CANTIDAD,DESCRIPCION,VALORUNITARIO,IMPORTE,REGISTRO,UNIDAD,IEPS,TASAIEPS,IVA,TASAIVA,TIPOFACTORIVA,TIPOFACTORIEPS,PYSSAT,UNISAT,DESCUENTO,GRUPOIEPS,PESO) VALUES ('" + frmPrincipal.EmisorBase + "','" + VSERIE + "','" + ELFOLIO.ToString + "','" + TXTNOTA.Text + "',@COD,@CANT,@DES,@PRE,@IMP,@REG,@UNI,@IEPS,@TIEPS,@IVA,@TIVA,@FIVA,@FIEPS,@PYSSAT,@UNISAT,@DESC,@GIEPS,@PESO)", CONZ)
        ...
        For X = 0 To DGV.Rows.Count - 1
            ...
            SQLD.Parameters("@PESO").Value = DGV.Item(16, X).Value
        ...
        If CHKCP.Checked Then
            ...
            SQLCCP.CommandTimeout = 600
            '" + frmPrincipal.EmisorBase + "','" + VSERIE + "','" + ELFOLIO.ToString + "'
            SQLCCP.Parameters.Add("@RFC", SqlDbType.VarChar).Value = frmPrincipal.EmisorBase
            SQLCCP.Parameters.Add("@SER", SqlDbType.VarChar).Value = VSERIE
            SQLCCP.Parameters.Add("@FOLIO", SqlDbType.Int).Value = ELFOLIO.ToString
            SQLCCP.Parameters.Add("@DIS", SqlDbType.Float).Value = DIS
            SQLCCP.Parameters.Add("@ORIFECHA", SqlDbType.DateTime).Value = ORIFECHA
            SQLCCP.Parameters.Add("@DESFECHA", SqlDbType.DateTime).Value = DESFECHA
            SQLCCP.Parameters.Add("@DESCP", SqlDbType.VarChar).Value = DESCP
            SQLCCP.Parameters.Add("@DESEDO", SqlDbType.VarChar).Value = DESEDO
            SQLCCP.Parameters.Add("@IDVEH", SqlDbType.Int).Value = IDVEH
            SQLCCP.Parameters.Add("@IDCHO", SqlDbType.Int).Value = IDCHO
            SQLCCP.Parameters.Add("@ASECAR", SqlDbType.VarChar).Value = ASECAR
            SQLCCP.Parameters.Add("@POLCAR", SqlDbType.VarChar).Value = POLCAR
            SQLCCP.Parameters.Add("@MONCAR", SqlDbType.VarChar).Value = MONCAR
            SQLCCP.ExecuteNonQuery()
            SQLCCP.Dispose()
        End If
    ...
    Private Sub GUARDARPREVIO()
        ...
        'MOSTRARREPORTE(REP, "Confirmación de Información de Cliente para Facturación", BDLlenatabla("SELECT * FROM TEMPVOBOCLIENTE WHERE RFC='" + frmPrincipal.EmisorBase + "' AND SERIE='" + VSERIE + "' AND FOLIO=" + ELFOLIO.ToString, CADENA), "")
    End Sub

    Dim DIS As Double
    Dim ORIFECHA As DateTime
    Dim DESFECHA As DateTime
    Dim DESCP As String
    Dim DESEDO As String
    Dim IDVEH As String
    Dim IDCHO As String
    ...
    Private Sub BTNGUARDAR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNGUARDAR.Click
        ...
        If CHKCP.Checked Then
            ...
            If VCCP.DialogResult = DialogResult.Yes Then
                DIS = VCCP.DIS
                ORIFECHA = VCCP.ORIFECHA
                DESFECHA = VCCP.DESFECHA
                DESCP = VCCP.DESCP
                DESEDO = VCCP.DESEDO
                IDVEH = VCCP.IDVEH
                IDCHO = VCCP.IDCHO
    ...
    Private Sub CARGAUSOCOMPROBANTE(ByVal REGIMEN As String)
        Try
            OPLlenaComboBox(CBUSO, LUC, "SELECT D.CLAVE,D.NOMBRE FROM CSATREGIMENUSO N INNER JOIN CSATUSOCOMPROBANTE D ON N.USO=D.CLAVE WHERE D.ACTIVO=1 AND N.REGIMEN='" + REGIMEN + "' ORDER BY NOMBRE", frmPrincipal.CadenaConexionFE, "Favor de Seleccionar", "")
        Catch ex As Exception
    ...
    Private Sub BTNAGREGAR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Dim VNCLIENTES As New frmClientesFacturasElectronicasV2
