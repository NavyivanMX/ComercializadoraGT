Public Class frmCancelaNotasCredito
    Dim CAJA, CAJERA, CLIENTE As Integer
    Dim Z As Integer

    Dim LPLATILLOS As New List(Of Integer)
    Private Sub frmCancelaNotasCredito_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub CARGANOTA()
        ...
        End If
        Dim QUERY As String
        QUERY = "SELECT E.NOMBRE EMPLEADO,N.FECHA,NOCAJA,N.CAJERA,C.NOMBRE,C.CLAVE FROM NOTASDEVENTACREDITO N INNER JOIN EMPLEADOS E  ON N.TIENDA=E.TIENDA AND N.CAJERA=E.CLAVE  INNER JOIN CLIENTES C ON C.TIENDA=N.TIENDA AND C.CLAVE=N.CLIENTE WHERE N.TIENDA='" + frmPrincipal.SucursalBase + "' AND N.NOTA=" + TXTNOTA.Text
        ...
        If LECTOR.Read Then
            ...
            If FEC.Date <> Now.Date Then
                MessageBox.Show("La nota que intenta modificar no coincide con la fecha actual, NO se permite eliminar", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
                Exit Sub
            Else
                ...
                'LEC.Close()
                'If FEC2 > FEC Then
                '    MessageBox.Show("La Nota Ha sido Incluida en Un Corte No se Permite Eliminar", "AVISO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                '    Exit Sub
                'End If
                TXTOBS.Focus()
            End If
        ...
        DGV.DataSource = BDLlenatabla("SELECT D.PRODUCTO CLAVE,P.NOMBRE,D.CANTIDAD,P.PRECIO,D.TOTAL FROM DETALLENOTASDEVENTACREDITO D INNER JOIN PRODUCTOS P ON D.PRODUCTO=P.CLAVE WHERE D.TIENDA='" + frmPrincipal.SucursalBase + "' AND D.NOTA=" + TXTNOTA.Text + "", frmPrincipal.CadenaConexion)
    ...
    Private Sub BTNELIMINAR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNELIMINAR.Click
        ...
        SQL.CommandText = "SPNOTASCANCELADASCREDITO"
        SQL.ExecuteNonQuery()
