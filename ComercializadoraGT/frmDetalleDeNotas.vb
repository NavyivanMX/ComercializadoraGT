Public Class frmDetalleDeNotas
    Dim CLACLI As New List(Of String)
    Dim CLANOTA As New List(Of String)
    Dim query As String
    Private Sub frmDetalleDeNotas_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub CARGADATOS()
        ...
        query = "SELECT P.NOMBRE PRODUCTO,D.CANTIDAD,PRECIO=CONVERT(NUMERIC(15,2),(D.TOTAL/D.CANTIDAD),2), D.DESCUENTO,D.TOTAL  FROM DETALLENOTASDEVENTA D INNER JOIN NOTASDEVENTA N ON N.TIENDA=D.TIENDA AND D.NOTA=N.NOTA INNER JOIN PRODUCTOS P ON P.CLAVE=D.PRODUCTO WHERE N.TIENDA='" + frmPrincipal.SucursalBase + "' AND N.NOTA=" + TXTNOTA.Text + "  "
        ...
        Dim SQLSELECT As New SqlClient.SqlCommand("SELECT N.FECHA,C.NOMBRE CLIENTE  FROM NOTASDEVENTA N INNER JOIN CLIENTES C ON N.TIENDA=C.TIENDA AND N.CLIENTE=C.CLAVE WHERE N.NOTA=" + TXTNOTA.Text + " AND N.TIENDA='" + frmPrincipal.SucursalBase + "'", frmPrincipal.CONX)
    ...
    Private Function IMPRIMIRNOTA(ByVal LANOTA As Integer) As Boolean
        ...
        Catch ex As Exception
            ...
            If xyz = 6 Then
                Return False
            Else
                MessageBox.Show("Utilice la opción reimprimir nota ( " + LANOTA.ToString + " )", "Información", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
                Return True
            End If
        End Try
        Return True
